...
public class AmexCachedLdapAuthorizationMap implements AuthorizationMap, InitializingBean, DisposableBean {
		...
		try {
			...
			if (refreshInterval == -1 && !refreshDisabled) {
				...
				for (PermissionType permissionType : PermissionType.values()) {
					...
					eventContext.addNamingListener(queueSearchBase, (new StringBuilder(searchPrefix)
							.append(getQueueNamesPrefix()).append(cnWildCardPrefix)).toString(), constraints,
							this.new CachedLDAPAuthorizationMapNamespaceChangeListener(DestinationType.QUEUE,
									permissionType));
				...
				for (PermissionType permissionType : PermissionType.values()) {
					...
					eventContext.addNamingListener(topicSearchBase, (new StringBuilder(searchPrefix)
							.append(getTopicNamesPrefix()).append(cnWildCardPrefix)).toString(), constraints,
							this.new CachedLDAPAuthorizationMapNamespaceChangeListener(DestinationType.TOPIC,
									permissionType));
				...
				for (PermissionType permissionType : PermissionType.values()) {
					...
					eventContext.addNamingListener(tempSearchBase, (new StringBuilder(searchPrefix)
							.append(getTempQueuesNamesPrefix())).toString(), constraints,
							this.new CachedLDAPAuthorizationMapNamespaceChangeListener(DestinationType.TEMP,
									permissionType));
	...
	protected void query() throws Exception {
		...
		for (PermissionType permissionType : PermissionType.values()) {
			try {
				...
				NamingEnumeration<SearchResult> search = currentContext.search(queueSearchBase, (new StringBuffer(
						searchPrefix).append(getQueueNamesPrefix()).append(cnWildCardPrefix)).toString(), constraints);
				LOG.trace("Search Results : " + search);
				processQueryResults(newMap, search, DestinationType.QUEUE, permissionType);

			} catch (Exception e) {
				LOG.error("Policy not applied!.  Error processing policy under '{}' with filter '{}'", new Object[] {
						queueSearchBase, getFilterForPermissionType(permissionType) }, e);
			}
		}
		...
		for (PermissionType permissionType : PermissionType.values()) {
			try {
				...
				processQueryResults(newMap, currentContext.search(topicSearchBase, (new StringBuffer(searchPrefix)
						.append(getTopicNamesPrefix()).append(cnWildCardPrefix)).toString(), constraints),
						DestinationType.TOPIC, permissionType);
			} catch (Exception e) {
				LOG.error("Policy not applied!.  Error processing policy under '{}' with filter '{}'", new Object[] {
						topicSearchBase, getFilterForPermissionType(permissionType) }, e);
			}
		}

		for (PermissionType permissionType : PermissionType.values()) {
			try {
				processQueryResults(newMap, currentContext.search(tempSearchBase, (new StringBuffer(searchPrefix)
						.append(getTempQueuesNamesPrefix()).append(cnWildCardPrefix)).toString(), constraints),
						DestinationType.TEMP, permissionType);
			} catch (Exception e) {
				LOG.error("Policy not applied!.  Error processing policy under '{}' with filter '{}'", new Object[] {
						tempSearchBase, getFilterForPermissionType(permissionType) }, e);
			}
		}
	...
	protected void processQueryResults(DefaultAuthorizationMap map, NamingEnumeration<SearchResult> results,
		...
		while (results.hasMore()) {
			...
			LOG.trace("Processing result:" + result);
						...
						while (memberAttributeEnum.hasMoreElements()) {
							...
							searchForAttributes(members,
									(new StringBuffer(memberDn).append(",").append(getPermissionSearchBase()))
											.toString(), result, permissionType);
						}
					}
				}
	...
	public void searchForAttributes(HashSet<Object> members, String memberDn, SearchResult result,
		...
		try {
			...
			memberAttributes = context.getAttributes(memberDn, new String[] { "objectClass", groupNameAttribute,
					userNameAttribute });
		...
		if ((!group && !user) || (group && user)) {
			LOG.error("Policy not applied! Can't determine type of member {} under entry {}", new Object[] { memberDn,
			...
			if (group && !user) {
				try {
					...
					NamingException ne = new NamingException("Can't create a group " + principalName + " of class "
							+ map1.getGroupClass());
					ne.initCause(e);
					throw ne;
				}
	 ...
	 * implementation expects a format of
	 * cn=<PERMISSION_NAME>,ou=<DESTINATION_PATTERN>,.... or
	 * ou=<DESTINATION_PATTERN>,.... for permission and destination entries,
	 * respectively. For example {@code cn=admin,ou=$,ou=...} or
	 * {@code ou=$,ou=...}.
	 * 
	 * @param dn
	 *            the DN to parse
	...
	protected String formatDestinationName(Rdn destinationName) {
		...
		if (destinationName.getValue().toString().toUpperCase().contains("ACTIVEMQ.ADVISORY")) {
			return "ActiveMQ.Advisory.>";
		}
	...
	protected void checkForUpdates() {
		...
		if (context == null
				|| (!refreshDisabled && (refreshInterval != -1 && System.currentTimeMillis() >= lastUpdated
						+ refreshInterval))) {
	...
	public void objectRenamed(NamingEvent namingEvent, DestinationType destinationType, PermissionType permissionType) {
		...
		try {
			...
			LOG.error("Policy not applied!  Error parsing DN for object rename for rename of {} to {}", new Object[] {
					oldBinding.getName(), newBinding.getName() }, e);
		} catch (Exception e) {
			LOG.error("Policy not applied!  Error processing object rename for rename of {} to {}", new Object[] {
					oldBinding.getName(), newBinding.getName() }, e);
		}
	}
	 ...
	 * destination types.
	 */
	protected class CachedLDAPAuthorizationMapNamespaceChangeListener implements NamespaceChangeListener,
