...
public class AmexCachedLdapAuthorizationMap implements AuthorizationMap, InitializingBean, DisposableBean {

	private static final Logger LOG = LoggerFactory.getLogger(AmexCachedLdapAuthorizationMap.class);
	...
	// the attribute name in which the the Destinations permissions are held
	private String permissionNameAttribute = "member";
	// The attribute name which holds the roles that has the permissions
	private String permissionGroupMemberAttribute = "member";
	private String permissionGroupNamePrefix;
	private int refreshInterval = 3600000;
	private boolean refreshDisabled = true;
	private boolean assignAdmintoWrite = false;
	...
	// Name prefix for any queue that will be created
	private String queueNamesPrefix;
	// Name prefix for any topic that will be created
	private String topicNamesPrefix;
	// Name prefix for any temporary destinations that will be created
	private String tempQueuesNamesPrefix;
	// Search prefix for all elements in LDAP
	private final String searchPrefix = "cn=";
	// WildCard used to search for a generic value in the LDAP tree
	private final String cnWildCardPrefix = "*";

	private final AtomicReference<DefaultAuthorizationMap> map = new AtomicReference<DefaultAuthorizationMap>(
			new DefaultAuthorizationMap());
	private final ThreadPoolExecutor updaterService;

	protected Map<ActiveMQDestination, AuthorizationEntry> entries = new ConcurrentHashMap<ActiveMQDestination, AuthorizationEntry>();

	private String storeType = "JKS";
	private String sslProtocol = "TLSv1";
	private String sslAlgorithm = "PKIX";
	private String trustStorePath;
	private String keyStorePath;
	private String trustStorePassword;
	private String keyStorePassword;
	/**
	 * To enable to create the destination cn's exactly matching the destination
	 * names in Activemq for wilds chars , instead of creating BAR.$ for a
	 * destination where where BAR-* is needed it can be created as BAR-$ in the
	 * Ldap
	 */

	private String pathSeparator = ".";
	private boolean createDestinationsWithPrefix = false;
	private final String EMPTY_STRING = "";

	public AmexCachedLdapAuthorizationMap() {
		// Allow for only a couple outstanding update request, they can be slow
		// so we
		// don't want a bunch to pile up for no reason.
		updaterService = new ThreadPoolExecutor(0, 1, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(2),
				new ThreadFactory() {

					@Override
					public Thread newThread(Runnable r) {
						return new Thread(r, "AmexCachedLdapAuthorizationMap update thread");
					}
				});
	...
	protected DirContext createContext() throws NamingException {
		...
		if (connectionURL.contains("ldaps")) {
			try {
				...
				final KeyStore trustStore = KeyStore.getInstance(getStoreType());
				final InputStream isTrs = new FileInputStream(trustStorePath);
				trustStore.load(isTrs, trustStorePassword.toCharArray());
				final TrustManagerFactory tmf = TrustManagerFactory.getInstance(getSslAlgorithm());
				tmf.init(trustStore);

				final SSLContext sc = SSLContext.getInstance(getSslProtocol());
				if (keyStorePath != null) {
					sc.init(kmf.getKeyManagers(), tmf.getTrustManagers(), new java.security.SecureRandom());
				} else {
					sc.init(null, tmf.getTrustManagers(), new java.security.SecureRandom());
				}
				ManagedSSLSocketFactory.setSocketFactory(sc.getSocketFactory());
	 ...
	 * listeners for push notifications if such an update style is enabled. This
	...
	protected DirContext open() throws NamingException {
		...
		try {
			...
			// For ActiveDirectory refreshInterval shoul always be set or
			// refreshDisabled should be always true
			// ADS does not support the NamespaceListener
			if (refreshInterval == -1 && !refreshDisabled) {
				...
				for (PermissionType permissionType : PermissionType.values()) {
					...
					eventContext.addNamingListener(queueSearchBase,
							(new StringBuilder(searchPrefix).append(getQueueNamesPrefix()).append(cnWildCardPrefix))
									.toString(),
							constraints, this.new CachedLDAPAuthorizationMapNamespaceChangeListener(
									DestinationType.QUEUE, permissionType));

				}
				// Listener for changes to the destination pattern entry itself
				// and not a permission entry.
				...
				for (PermissionType permissionType : PermissionType.values()) {
					...
					eventContext.addNamingListener(topicSearchBase,
							(new StringBuilder(searchPrefix).append(getTopicNamesPrefix()).append(cnWildCardPrefix))
									.toString(),
							constraints, this.new CachedLDAPAuthorizationMapNamespaceChangeListener(
				...
				eventContext.addNamingListener(topicSearchBase, "objectClass=*", new SearchControls(),
				...
				for (PermissionType permissionType : PermissionType.values()) {
					...
					eventContext.addNamingListener(tempSearchBase,
							(new StringBuilder(searchPrefix).append(getTempQueuesNamesPrefix())).toString(),
							constraints, this.new CachedLDAPAuthorizationMapNamespaceChangeListener(
			...
			context = null;
			LOG.error("Making the context null :", e);
			throw e;
		}
	...
	/**
	 * Queries the directory and initializes the policy based on the data in the
	 * directory. This implementation should not be invoked concurrently.
	 * 
	 * @throws Exception
	 *             if there is an unrecoverable error processing the directory
	 *             contents
	 */
	...
	protected void query() throws Exception {
		...
		for (PermissionType permissionType : PermissionType.values()) {
			try {
				...
				NamingEnumeration<SearchResult> search = currentContext.search(queueSearchBase,
						(new StringBuffer(searchPrefix).append(getQueueNamesPrefix()).append(cnWildCardPrefix))
								.toString(),
						constraints);

				processQueryResults(newMap, search, DestinationType.QUEUE, permissionType);

			} catch (Exception e) {
				LOG.error("Policy not applied!.  Error processing policy under '{}' with filter '{}'",
						new Object[] { queueSearchBase, getFilterForPermissionType(permissionType) }, e);
			}
		}

		for (PermissionType permissionType : PermissionType.values()) {
			try {
				NamingEnumeration<SearchResult> search = currentContext.search(topicSearchBase,
						(new StringBuffer(searchPrefix).append(getTopicNamesPrefix()).append(cnWildCardPrefix))
								.toString(),
						constraints);

				processQueryResults(newMap, search, DestinationType.TOPIC, permissionType);
			} catch (Exception e) {
				LOG.error("Policy not applied!.  Error processing policy under '{}' with filter '{}'",
						new Object[] { topicSearchBase, getFilterForPermissionType(permissionType) }, e);
			}
		}

		for (PermissionType permissionType : PermissionType.values()) {
			try {
				NamingEnumeration<SearchResult> search = currentContext.search(tempSearchBase,
						(new StringBuffer(searchPrefix).append(getTempQueuesNamesPrefix()).append(cnWildCardPrefix))
								.toString(),
						constraints);
				processQueryResults(newMap, search, DestinationType.TEMP, permissionType);
			} catch (Exception e) {
				LOG.error("Policy not applied!.  Error processing policy under '{}' with filter '{}'",
						new Object[] { tempSearchBase, getFilterForPermissionType(permissionType) }, e);
			}
		}
	...
	/**
	 * Processes results from a directory query in the context of a given
	 * destination type and permission type. This implementation should not be
	 * invoked concurrently.
	 * 
	 * @param results
	 *            the results to process
	 * @param destinationType
	 *            the type of the destination for which the directory results
	 *            apply
	 * @param permissionType
	 *            the type of the permission for which the directory results
	 *            apply
	 * 
	 * @throws Exception
	 *             if there is an error processing the results
	 */
	protected void processQueryResults(DefaultAuthorizationMap map, NamingEnumeration<SearchResult> results,
	 ...
	 * Relevant for synchronous policy updates. This implementation should not
	 ...
	 * the DN in {@code dn}. This implementation should not be invoked
	 * concurrently.
	 * 
	 * @param map
	 ...
	 *             if destination type is not one of
	 *             {@link DestinationType#QUEUE}, {@link DestinationType#TOPIC},
	 *             {@link DestinationType#TEMP} or if the policy entry DN is
	 *             malformed
	 */
	protected AuthorizationEntry getEntry(DefaultAuthorizationMap map, LdapName dn, DestinationType destinationType) {
		...
		case TEMP:
			// handle temp entry
			// TODO As the structure is now flat is is necessary to check for
			// the dn depth ?
	...
	/**
	 * Applies the policy from the directory to the given entry within the
	 * context of the provided permission type.
	 * 
	 * @param entry
	 ...
	 */
	protected void applyACL(AuthorizationEntry entry, SearchResult result, PermissionType permissionType)
			throws NamingException {

		// Find members

		Attribute memberAttributeG = result.getAttributes().get(permissionNameAttribute);

		if (memberAttributeG != null) {
			NamingEnumeration<?> memberAttributeEnumG = memberAttributeG.getAll();
			SearchControls searchControls = new SearchControls();
			searchControls.setSearchScope(SearchControls.SUBTREE_SCOPE);
			NamingEnumeration<SearchResult> search = null;

			HashSet<Object> members = new HashSet<Object>();
			while (memberAttributeEnumG.hasMoreElements()) {

				String memberDn = (String) memberAttributeEnumG.nextElement();

				// To check if this is full dn path , in that case attributes
				// can be fetched directly instead of a search
				if (!legacyGroupMapping) {
					// Each of the member under the Actual Destination will be
					// mapped to a permission group , we will search for
					// attributes
					// under the group straight
					// using context
					if (memberDn != null && memberDn.toUpperCase().contains(permissionType.name().toUpperCase())) {
						try {
							Attributes attributes = context.getAttributes(memberDn,
									new String[] { permissionGroupMemberAttribute });
							NamingEnumeration<?> all = attributes.get(permissionGroupMemberAttribute).getAll();

							while (all.hasMore()) {
								memberDn = (String) all.nextElement();
								searchForAttributes(members, memberDn, result, permissionType);
							}
						} catch (Exception e) {
							LOG.error(e.getMessage(), e);
						}
					...
					if (memberDn != null && memberDn.toUpperCase().contains(permissionType.name().toUpperCase())) {
						try {
							search = context.search(getPermissionSearchBase(), memberDn, searchControls);
						} catch (Exception e) {
							LOG.error("Unable to search for permission groups :" + e.getMessage(), e);
						}
						SearchResult searchResult = search.nextElement();
						Attribute memberAttribute = searchResult.getAttributes().get(permissionNameAttribute);
						NamingEnumeration<?> memberAttributeEnum = memberAttribute.getAll();
						while (memberAttributeEnum.hasMoreElements()) {
							memberDn = (String) memberAttributeEnum.nextElement();
							searchForAttributes(members,
									(new StringBuffer(memberDn).append(",").append(getPermissionSearchBase()))
											.toString(),
			...
			try {
				...
				LOG.error("Policy not applied! Error adding principals to ACL under {}",
						new Object[] { result.getNameInNamespace() }, e);
			}
		} else {
			LOG.error("Unable to get the attribute {} for Element {} ..ignoring exception and moving on..",
					new Object[] { permissionNameAttribute, result.getName() });

		}

	}

	public void searchForAttributes(HashSet<Object> members, String memberDn, SearchResult result,
		...
		// Lookup of member to determine principal type (group or user) and
		// name.
		Attributes memberAttributes = null;
		try {
		...
		while (memberEntryObjectClassAttributeEnum.hasMoreElements()) {
			...
			if (objectClass.equalsIgnoreCase(groupObjectClass)) {
				...
				if (name == null) {
					LOG.error("Policy not applied! Group {} does not have name attribute {} under entry {}",
							new Object[] { memberDn, groupNameAttribute, result.getNameInNamespace() });
			...
			if (objectClass.equalsIgnoreCase(userObjectClass)) {
				...
				if (name == null) {
					LOG.error("Policy not applied! Group {} does not have name attribute {} under entry {}",
							new Object[] { memberDn, userNameAttribute, result.getNameInNamespace() });
					break;
				}
		...
		if ((!group && !user) || (group && user)) {
			LOG.error("Policy not applied! Can't determine type of member {} under entry {}",
					new Object[] { memberDn, result.getNameInNamespace() });

		} else if (principalName != null) {
			DefaultAuthorizationMap map1 = this.map.get();
			if (group && !user) {
				try {

					Object createGroupPrincipal = DefaultAuthorizationMap.createGroupPrincipal(principalName,
							map1.getGroupClass());

					members.add(createGroupPrincipal);
				} catch (Exception e) {
					NamingException ne = new NamingException(
							"Can't create a group " + principalName + " of class " + map1.getGroupClass());
					ne.initCause(e);
					throw ne;
				}
			} else if (!group && user) {
	...
	/**
	 * Applies policy to the entry given the actual principals that will be
	 * applied to the policy entry.
	 * 
	 * @param entry
	...
	protected void applyAcl(AuthorizationEntry entry, PermissionType permissionType, Set<Object> acls) {
		...
		case WRITE:
			...
			// Request to have this in place,
			// Generally not advisable , default will be always false
			if (isAssignAdmintoWrite()) {
		...
		if (entry != null && entry.getDestination() != null) {
			LOG.trace(permissionType.name() + " : Destination   " + entry.getDestination().getPhysicalName()
					+ " ACLS : " + acls);
		} else {
			LOG.trace(permissionType.name() + " : Destination   "
					+ ((TempDestinationAuthorizationEntry) entry).getValue().toString() + " ACLS : " + acls);
		}
	}

	/**
	 * Parses a DN into the equivalent {@link ActiveMQDestination}. The default
	 * implementation expects a format of cn=<PERMISSION_NAME>,ou=
	 * <DESTINATION_PATTERN>,.... or ou=<DESTINATION_PATTERN>,.... for
	 * permission and destination entries, respectively. For example
	 * {@code cn=admin,ou=$,ou=...} or {@code ou=$,ou=...}.
	 * 
	 * @param dn
	 *            the DN to parse
	 * @param destinationType
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             if {@code destinationType} is {@link DestinationType#TEMP} or
	 *             if the format of {@code dn} is incorrect for for a topic or
	 *             queue
	 * 
	...
	protected ActiveMQDestination formatDestination(LdapName dn, DestinationType destinationType) {
		...
		case QUEUE:
		case TOPIC:
			// There exists a need to deal with both names representing a
			// permission or simply a
			// destination. As such, we need to determine the proper RDN to work
			// with based
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             if {@code destinationType} is not one of
	 *             {@link DestinationType#TOPIC} or
	...
	protected ActiveMQDestination formatDestination(Rdn destinationName, DestinationType destinationType) {
		...
		switch (destinationType) {
		case QUEUE:
			dest = new ActiveMQQueue(formatDestinationName(destinationName, getQueueNamesPrefix()));
			break;
		case TOPIC:
			dest = new ActiveMQTopic(formatDestinationName(destinationName, getTopicNamesPrefix()));
			break;
		default:
			throw new IllegalArgumentException("Unknown destination type: " + destinationType);
		}
	...
	private String formatDestinationName(Rdn destinationName, String prefix) {

		String destination = destinationName.getValue().toString().replaceAll(ANY_DESCENDANT, ">");
		if (!isCreateDestinationsWithPrefix()) {
			destination = destination.replaceAll(prefix, EMPTY_STRING);
		}
		if (destination.toUpperCase().contains("ADVISORY")) {
			destination = "ActiveMQ.Advisory.>";
		}

		if (!getPathSeparator().equalsIgnoreCase(".")) {
			String[] split = destination.split(getPathSeparator());
			destination = DestinationPath.toString(split);
	...
	/**
	 * Parses the RDN representing a destination name/pattern into the standard
	 * string representation of the name/pattern. This implementation does not
	 * care about the type of the RDN such that the RDN could be a CN or OU.
	 * 
	 * @param destinationName
	 *            the RDN representing the name or pattern for the destination
	 * 
	...
	/**
	 * Performs a check for updates from the server in the event that
	 * synchronous updates are enabled and are the refresh interval has elapsed.
	 */
	...
	/**
	 * Provides synchronized and defensive access to the admin ACLs for temp
	 * destinations as the super implementation returns live copies of the ACLs
	 * and {@link AuthorizationEntry} is not setup for concurrent access.
	 */
	@Override
	...
	/**
	 * Provides synchronized and defensive access to the read ACLs for temp
	 * destinations as the super implementation returns live copies of the ACLs
	 * and {@link AuthorizationEntry} is not setup for concurrent access.
	 */
	@Override
	...
	/**
	 * Provides synchronized and defensive access to the write ACLs for temp
	 * destinations as the super implementation returns live copies of the ACLs
	 * and {@link AuthorizationEntry} is not setup for concurrent access.
	 */
	@Override
	...
	public void objectRemoved(NamingEvent namingEvent, DestinationType destinationType, PermissionType permissionType) {
		...
		try {
			...
			LOG.error("Policy not applied!  Error parsing DN for object removal for removal of {}", result.getName(),
					e);
		} catch (Exception e) {
	...
	/**
	 * Handler for renamed policy entries in the directory. This handler deals
	 * with the renaming of destination entries as well as permission entries.
	 * If the permission type is not null, it is assumed that we are dealing
	 * with the renaming of a permission entry. Otherwise, it is assumed that we
	 * are dealing with the renaming of a destination entry.
	 * 
	 * @param namingEvent
		...
		try {
			...
			if (permissionType != null) {
				...
				for (PermissionType newPermissionType : PermissionType.values()) {
					NamingEnumeration<SearchResult> results = context.search(newName,
			...
			LOG.error("Policy not applied!  Error parsing DN for object rename for rename of {} to {}",
					new Object[] { oldBinding.getName(), newBinding.getName() }, e);
		} catch (Exception e) {
			LOG.error("Policy not applied!  Error processing object rename for rename of {} to {}",
					new Object[] { oldBinding.getName(), newBinding.getName() }, e);
		}
	}
	...
	public void namingExceptionThrown(NamingExceptionEvent namingExceptionEvent) {
		context = null;
		LOG.error("Caught unexpected exception." + namingExceptionEvent.getException().getMessage(),
	...
	protected class CachedLDAPAuthorizationMapNamespaceChangeListener
		...
		/**
		 * Creates a new listener. If {@code permissionType} is {@code null},
		 * add and remove events are ignored as they do not directly affect
		 * policy state. This configuration is used when listening for changes
		 * on entries that represent destination patterns and not for entries
		 * that represent permissions.
		 * 
		 * @param destinationType
		 ...
		 */
		public CachedLDAPAuthorizationMapNamespaceChangeListener(DestinationType destinationType,
		...
		public void namingExceptionThrown(NamingExceptionEvent evt) {
			AmexCachedLdapAuthorizationMap.this.namingExceptionThrown(evt);
		}

		@Override
		public void objectAdded(NamingEvent evt) {
			// This test is a hack to work around the fact that Apache DS 2.0
			// seems to trigger notifications
			// for the entire sub-tree even when one-level is the selected
			// search scope.
			if (permissionType != null) {
				AmexCachedLdapAuthorizationMap.this.objectAdded(evt, destinationType, permissionType);
			}
		}
		...
		public void objectRemoved(NamingEvent evt) {
			...
			// search scope.
			if (permissionType != null) {
				AmexCachedLdapAuthorizationMap.this.objectRemoved(evt, destinationType, permissionType);
			}
		}

		@Override
		public void objectRenamed(NamingEvent evt) {
			AmexCachedLdapAuthorizationMap.this.objectRenamed(evt, destinationType, permissionType);
		}
		...
		public void objectChanged(NamingEvent evt) {
			...
			// search scope.
			if (permissionType != null) {
				AmexCachedLdapAuthorizationMap.this.objectChanged(evt, destinationType, permissionType);
			}
		}
	}
	...
	public String getTempQueuesNamesPrefix() {
		return tempQueuesNamesPrefix;
	}

	public void setTempQueuesNamesPrefix(String tempQueuesNamesPrefix) {
		this.tempQueuesNamesPrefix = tempQueuesNamesPrefix;
	}

	public boolean isAssignAdmintoWrite() {
		return assignAdmintoWrite;
	}

	public void setAssignAdmintoWrite(boolean assignAdmintoWrite) {
		this.assignAdmintoWrite = assignAdmintoWrite;
	}

	public String getPermissionGroupNamePrefix() {
		return permissionGroupNamePrefix;
	}

	public void setPermissionGroupNamePrefix(String permissionGroupNamePrefix) {
		this.permissionGroupNamePrefix = permissionGroupNamePrefix;
	}
	...
	public boolean isCreateDestinationsWithPrefix() {
		return createDestinationsWithPrefix;
	}

	public void setCreateDestinationsWithPrefix(boolean createDestinationsWithPrefix) {
		this.createDestinationsWithPrefix = createDestinationsWithPrefix;
	}
